               ...
               <option v-for="option in options" :value="option.code"  :data-content=" `<div style='display: flex; align-items: center;'>  ` +  option.name + `&emsp;` + `<img src='` + option.img + `'` + `class=\'asdasd\'>` +`</div>` " >
                <!-- /> --> 
                  ...
                  <select  id="countries" class="form-control selectpicker sp2" data-width="300px" data-size="7"  data-live-search="true"  @change="onChange2()"   name="selectedOption" v-model="selectedOption">
                    <option v-for="option in options2" :value="option.country" >
...
this.tientra=this.tientra1*this.socmt*( 100 - this.chietkhau )/ 100 ;
    ...
    onChange() {
this.options2 = [
        { name: 'Chờ xíu nhé, Đang tải danh sách', country: 1 , cost: 0 , count: 1 } ,
        { name: 'Chờ xíu nhé, Đang tải danh sách', country: 2 , cost: 1 , count: 2 }
      ] ;
      setTimeout(() => {    
                    $('.selectpicker').selectpicker('refresh');
                    setTimeout(() => {     
                        $('.sp2').selectpicker('refresh');  }, 300)
                }, 300) ,
    ...
    onChange2() {
        ...
        if(!this.selectedOption){
                    alert('Đã xảy ra lỗi vui lòng liên hệ admin')
                    return
                }
                else if ( this.selectedOption.count == '' || this.selectedOption.count < 1 )
                {
                    alert('Dịch vụ này đã hết số để thuê')
                    return
                }
                else
                {
                    console.log(this.selectedOption.country)
      console.log(this.selectedOption.cost)
      console.log(this.selectedOption.count)
                }
...
for (const [country, services] of Object.entries(options2ok)) {
