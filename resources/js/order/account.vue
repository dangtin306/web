          ...
          <div class='max-w-md mx-auto  space-y-6'>
            ...
            <div class="col-lg-6">
              <div class="form-group">
                <label for="countries2" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Chọn nền
                  tảng</label>

                <select id="countries2" class="
                        selectpicker sp1 form-control" data-size="5" data-live-search="true" @change="onChange()"
                  name="nhamang" v-model="chedomuaacc">
    ...
    <div v-if="hientkclone == 1 && damualendau == true">
      <div class="relative py-3 sm:max-w-xl sm:mx-auto">
        <div class="flex flex-col items-center justify-center py-2">
          <div
            class="absolute inset-0 bg-gradient-to-r from-indigo-200 to-purple-600 shadow-lg transform rotate-6 rounded-3xl">
      ...
      <table class="table-fixed relative w-full border border-separate text-break text-sm">
        ...
        <tbody>
          <template v-for="item in VIETTEL">
            ...
            <tr class="bg-gradient-to-r from-blue-100 to-pink-200 hover:from-pink-400 hover:to-yellow-300 lg:text-black"
              v-if="!item.tongdai">
              <td class="py-3 px-2 font-medium capitalize">
                <div v-html="item.name"></div>
              </td>
              <td class="py-3 px-2">{{ parseFloat((item.price * 1.3).toFixed(2)) !== parseInt((item.price * 1.3)) ?
                (item.price * 1.3).toFixed(2) : parseInt((item.price * 1.3)) }}
              </td>
              <td class="py-3 px-2"> {{ item.amount }} </td>
              <td class="py-3 px-2">

                <div v-bind:id="item.id">
                  <div v-html="item.description"></div> <span @click="saochepp(item.id)">
                    <div class="float-right">
                      <button class='text-xs font-medium rounded-full px-4 py-2 space-x-1 bg-white text-black'>
                        <div v-if="idservice == item.id">{{ dasaochep }}</div>
                        <div v-else> Mua ngay</div>
                      </button>
                    </div>
                  </span>
                </div>
