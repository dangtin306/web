            ...
            <div class='w-full max-w-lg px-10 py-3 mx-auto bg-white rounded-lg shadow-xl'>
              ...
              <div ref="socialpro" class='max-w-md mx-auto  space-y-5'>
                ...
                <div class="form-group">
                  ...
                  <label for="social"
                    class="text-center block mb-1 text-sm font-medium text-gray-900 dark:text-gray-400">Chọn nền
                    tảng</label>
                ...
                selectpicker sp1 " data-style=" absdakjsbdka hover:bg-red-300 rounded" name="nhamang" v-model="social">
                  ...
                  <div v-if="chedook != null && chedook != 2 && chedook != 3">
                    <label for="countries"
                      class="text-center block mb-1 text-sm font-medium text-gray-900 dark:text-gray-400">Chọn thể
                      loại</label>
                    <select id="countries"
                      data-style="bg-pink-200 sdsasdasdasdasd hover:bg-red-300 font-bold py-2 px-4 rounded"
                      class="form-control selectpicker sp2" @change="onChange2truoc()" name="theloai" v-model="theloai">
                      <option v-for="option in options2" :value="`${option.value}+${option.uri}`"
                  ...
                  <div v-if="chedook != null">
                    <label for="category"
                      class="text-center block mb-1 text-sm font-medium text-gray-900 dark:text-gray-400">Chọn chuyên
                      mục</label>
                    <select data-header="Chọn 1 chuyên mục" id="category"
                      ...
                      <option v-for="option in options3" :value="option.code"
                        :data-content="`<div style='display: flex; align-items: center;'>${option.name}&nbsp;${option.img ? `<img src='${option.img}' class='asdasd'>` : ''}</div>`">
                      </option>
                  ...
                  <div v-if="chondichvu == 1" x-show="showen">
                    <label for="dichvu"
                      class="text-center block mb-1 text-sm font-medium text-gray-900 dark:text-gray-400">Chọn dịch
                      vụ</label>
                    <div @click="onChange4()">
                      <select ref="dichvu"
                        data-style="bg-pink-200 adasdasdasdadacas hover:bg-red-300 font-bold py-2 px-4 rounded"
                        id="dichvu" class="form-control selectpicker sp4  " show-tick data-size="6"
                        data-live-search="true" data-live-search-placeholder="tìm kiếm.." name="dichvu" v-model="dichvu">
                ...
                <input class="mt-2 border-3 form-control-pink form-control form-control-alternative" v-model="lienketchay"
                  placeholder="Copy và điền liên kết ở đây" />
                ...
                <div v-if="timeuoctinh" class="mt-2 form-group fields" style="display: block;">
                  <div v-if="ádasdacsacas == 1" class="col mt-0">
                    <div @click="adbasdasdandsa2"
                      class="mt-4 mx-2 items-center justify-center from-teal-100 via-teal-300 to-teal-500">
                  ...
                  <label v-if="timeuoctinh" class="control-label">thời gian ước tính
                    ...
                    <button @click="adbasdasdandsa" class="ml-1 mr-1 " data-bs-toggle="tooltip" data-bs-placement="right"
                      title="Thời gian hoàn thành trung bình 10 đơn hàng gần nhất trên 500 đơn vị">
...
border-3 form-control-pink form-control form-control-alternative" v-model="soluongtang"
                    placeholder="Nhập số lượng mong muốn" />
                  <span class="text-xs">
                    Tối thiểu {{ minorder }}
                    &emsp;
                    Tối đa {{ maxorder }}
  ...
  border-3 form-control-pink form-control form-control-alternative" v-model="idbinhluan"
                    placeholder="id người viết bình luận" />


                </div>

                <div v-if="typedichvu == 'comment'" x-show="showen" class="mt-3 space-y-2 text-gray-700"
                  x-data="{isshow:false}">
                  ...
                  <div class="relative  focus-within:text-gray-900 dark:focus-within:text-gray-800 ">
                    ...
                    <div aria-hidden="true" class="absolute inset-y-0 flex items-center px-4 pointer-events-none">
                      <svg aria-hidden="true" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none"
...
focus:ring-gray-800 ring-gray-400 ring focus:ring-offset-2 focus:ring-offset-white dark:border-gray-600 dark:bg-primary-darker
 dark:focus:ring-offset-dark-eval-1 block w-full" id="name2" name="name2" required="required" v-model="name2"
                      @keyup="countLine" autocomplete="new-password" placeholder="Nhập mỗi nội dung 1 dòng" type="text">
</textarea>


                    <div class="absolute z-30 inset-y-1 flex items-center px-4 ">
                      <button type="button" class="z-30 ">
                        <svg x-show="!isshow" aria-hidden="true" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg"
                        ...
                        </svg> <svg x-show="isshow" aria-hidden="true" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="max-h-16">
                  <div class="cards">
                    ...
                    <Adsense data-ad-client="ca-pub-4574266110812955" data-ad-slot="8795043992"
                  ...
                  <section class=" flex flex-col items-start justify-end gap-1">
                    <div class="flex items-center">
                      <section class="w-6 flex flex-col items-center justify-start">
                        ...
                        {{ parseFloat((tientra2).toFixed(2)) !== parseInt((tientra2)) ? (tientra2).toFixed(2) :
                          parseInt((tientra2)) }}
                      </h1>
                    </div>
                    ...
                    <p class="text-sm font-medium text-zinc-400 antialiased">
                      ...
                      <span v-if="chietkhau != null && chietkhau > 0"
                        ...
                        <br>
                        Giá ưu đãi giảm còn :
                        {{ parseFloat((tientra).toFixed(2)) !== parseInt((tientra)) ? (tientra).toFixed(2) :
                          parseInt((tientra)) }}
                      </span>
...
export default {
  ...
  data() {
    return {
      ...
      options: [
        ...
        { text: 'Quảng cáo Website', value: [{ social: 'website', chedo: 7 }], img: 'https://hust.media/img/icon/webicon.png' },
  ...
  methods: {
    ...
    onChange() {
      ...
      if (this.socialok == 'website') {
        window.location = "https://hust.media/shop/orderapp";
      }
      else if (this.chedook == 1) {
