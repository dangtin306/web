...
img.asdasd {
  ...
  input:checked ~ .dot {
    ...
    background-color: #ffa5ec !important;
  }
  input:checked ~ .dot2 {
    ...
    <div  class='mt-4 mx-2 items-center justify-center from-teal-100 via-teal-300 to-teal-500 '>
      ...
      <div  class='w-full max-w-lg px-10 py-8 mx-auto bg-white rounded-lg shadow-xl'>
          ...
          <div class='max-w-md mx-auto  space-y-6'>
                ...
                selectpicker sp1 form-control"   @change="onChange()" data-width="auto" data-size="7"  data-live-search="true"    name="nhamang" v-model="social">
               <option v-for="option in options" :value="option.code"  :data-content=" `<div style='display: flex; align-items: center;'>  ` +  option.name + `&emsp;` + `<img src='` + option.img + `'` + `class=\'asdasd\'>` +`</div>` " >
                <!-- /> --> 
        
                    </option>
                  </select>
                  <br>
               <div  v-if="options2 != null ">
 <label for="countries" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Chọn dịch vụ
    
 </label>
 
                  <select  id="countries" class="form-control selectpicker sp2" data-width="300px" data-size="7"  data-live-search="true"  @change="onChange2()"     name="selectedOption" v-model="selectedOption">
                    <option v-for="option in options2" 
                    :value="option.country" >
             <h3> {{ option.name }}</h3>     giá  {{ option.cost*330*1.3 }} Xu     còn lại {{ option.count }}
                    </option>
                  </select>
               </div>
               <br>
               <label v-if="maxorder == 1" for="toggleB" class="flex items-center cursor-pointer">
...
Dịch vụ {{minorder }} 
        ...
        <div class="max-w-2xl px-1 py-4  mx-auto rounded-lg shadow-xl">
            <div v-if="laylichsudoithe == null" class="container-fluid">
...
import VRuntimeTemplate from "vue3-runtime-template";
  ...
  // },
      ...
      data (){
            return {
              ...
              name2 : '',
                ...
                selected: null ,
      options: [
        { name: 'Đang tải danh sách', code: 'null' , img: 'null' } 
      ],
      options2: null ,
      options3: null ,
      dichvu: null ,
    ...
    .catch(error => console.log(error) ,
      ...
      if (this.isToggleOn) {
        this.options2.sort((a, b) => a.cost - b.cost) ;
...
console.log(this.options2);
     setTimeout(() => {    
        ...
        this.options2.sort((a, b) => a.top - b.top) ;
this.options2 = this.options2 ;
    ...
    toggleButton2() {
        ...
        setTimeout(() => {    
         ...
         .post('https://tecom.pro/otp/themotp.php', {
          ...
          namesocial : this.namesocial ,
      namecountry: this.minorder
   ...
   Swal.fire(this.message ) ;
    ...
    onChange() {
        ...
        setTimeout(() => {    
                 ...
                 $('.sp2').selectpicker('refresh');
                 this.options2 = [
        { name: 'Chờ xíu nhé, Đang tải danh sách', country:null , cost: null , count: null  } 
      ] ;
                 setTimeout(() => {     
                    $('.sp2').selectpicker('refresh'); }, 200)
             }, 200) ;
    ...
    if(this.selectedOption == '' && this.selectedOption != 0   ){
                    alert('Lỗi lấy thông tin dịch vụ')
                    return
                }
 else
 {
    const selected = this.options2.find((option) => option.country === this.selectedOption);
this.service = selected.service;
this.minorder = selected.name;
const cost = selected.cost;
const count = selected.count;
const selected2 = this.options.find((option2) => option2.code === this.social);
this.namesocial = selected2.name;
console.log(this.namesocial );
if( count == '' || count < 1 ){
                    alert('Dịch vụ này đã hết số')
                    return
                }
                else
                {
this.tientra = cost * 330 * 1.3 ;
  ...
  return cookie[cookieName];
    ...
    {
        this.maxorder = 1 ;
      const options2ok = response.data ;
    //   const newArray = [];

const newArray = Object.keys(options2ok).map((key, index) => {
  const item = options2ok[key]
  return {
    top: index,
...
for (let obj of newArray) {
  const id = obj.country;
  if (this.listcountry.hasOwnProperty(id)) {
    obj.name = this.listcountry[id].eng;
  }
}
