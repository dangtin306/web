 ...
 */
 const debounce = (func, wait, immediate) => {
  ...
  var script = {
    ...
    methods: {
      init() {
        ...
        this.$emit("move", {
          stack: this.stack,
          direction: this.isDraggingRight ? 'right' : 'left',
          isDragging: this.isDragging
        });
        this.$emit("update", {
          active: this.stack[1]
  ...
  const __vue_inject_styles__ = function (inject) {
    if (!inject) return;
    inject("data-v-241c6e7b_0", {
      source: ".vue-card-stack__wrapper[data-v-241c6e7b]{position:relative}.vue-card-stack__stack[data-v-241c6e7b]{position:relative;overflow:hidden}.vue-card-stack__card[data-v-241c6e7b]{position:absolute;transform-origin:0 50%;cursor:grab}",
      map: undefined,
      media: undefined
    });
  };
  ...
  const __vue_scope_id__ = "data-v-241c6e7b";
  /* module identifier */
